#!/usr/bin/env python3
"""
Evaluate trained XGBoost ranker model from prod_train_ranker.py

This script evaluates ranking performance using NDCG@K, Precision@K, and MAP@K metrics
with proper grouped evaluation by date. It automatically finds the latest model files
or allows manual specification of model artifacts.
"""
...
def find_latest_model_files(model_dir: str = "../model_output") -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """Find the latest model, features, and sharpe edges files from prod_train_ranker.py output."""
    model_pattern = os.path.join(model_dir, "xgboost_ranker2_*_*.joblib")
    feature_pattern = os.path.join(model_dir, "xgb_feature_names_*_*.pkl")
    edges_pattern = os.path.join(model_dir, "sharpe_qcut_edges_*_*.pkl")
    
    model_files = glob.glob(model_pattern)
    feature_files = glob.glob(feature_pattern)
    edges_files = glob.glob(edges_pattern)
    ...
    if not feature_files:
        logger.error(f"No feature files found matching pattern: {feature_pattern}")
        return None, None, None
        
    if not edges_files:
        logger.error(f"No sharpe edges files found matching pattern: {edges_pattern}")
        return None, None, None
    ...
    return latest_model, latest_features, latest_edges
    ...
    try:
        ...
        if 'contractID' not in df.columns:
             if 'contract_id' in df.columns: 
                 ...
                 raise ValueError("Missing 'contractID' or equivalent column for grouping.")
...
def calculate_target_eval(df, lookahead_days=TARGET_LOOKAHEAD_DAYS):
    ...
    if 'contractID' not in df.columns or 'last' not in df.columns:
        logger.error("Cannot calculate target: 'contractID' or 'last' column missing after preprocessing.")
...
def evaluate_model(args):
    ...
    if df_eval_processed_utils.empty: 
        return False
    logger.info(f"Shape after utils.preprocess_data: {df_eval_processed_utils.shape}")

    logger.info("Engineering basic features (Price/IV Changes)...")
    df_eval_features_intermediate = df_eval_processed_utils.sort_values(by=['contractID', 'date'])
    if 'last' in df_eval_features_intermediate.columns:
        last_shifted = df_eval_features_intermediate.groupby('contractID')['last'].shift(1)
        df_eval_features_intermediate['price_change_1d'] = np.where(
            last_shifted.fillna(0) != 0, 
            (df_eval_features_intermediate['last'] - last_shifted) / last_shifted, 
            0
        )
    else: 
        df_eval_features_intermediate['price_change_1d'] = 0
    
    if 'implied_volatility' in df_eval_features_intermediate.columns:
        df_eval_features_intermediate['iv_change_1d'] = df_eval_features_intermediate.groupby('contractID')['implied_volatility'].diff(1)
    else: 
        df_eval_features_intermediate['iv_change_1d'] = 0
    
    df_eval_features_intermediate['price_change_1d'] = df_eval_features_intermediate['price_change_1d'].fillna(0)
    df_eval_features_intermediate['iv_change_1d'] = df_eval_features_intermediate['iv_change_1d'].fillna(0)
    logger.info(f"Shape after basic feature engineering: {df_eval_features_intermediate.shape}")

    df_eval_with_target = calculate_target_eval(df_eval_features_intermediate, TARGET_LOOKAHEAD_DAYS)
    ...
    for k_val in K_VALUES:
        ...
        performance_summary[f'NDCG@{k_val}'] = mean_ndcg
        ...
        performance_summary[f'Precision@{k_val}'] = mean_precision
        ...
        performance_summary[f'MAP@{k_val}'] = mean_ap
        logger.info("---")
    
    # Performance assessment
    ndcg_20 = performance_summary.get('NDCG@20', 0.0)
    ndcg_5 = performance_summary.get('NDCG@5', 0.0)
    ...
    if ndcg_20 > 0.75:
        logger.info(f"   🏆 EXCELLENT ranking performance!")
        return True
    elif ndcg_20 > 0.60:
        logger.info(f"   ✅ GOOD ranking performance")
        return True
    elif ndcg_20 > 0.50:
        logger.info(f"   ⚠️  FAIR ranking performance")
        return True
    else:
        logger.info(f"   ❌ POOR ranking performance")
        return False

def main():
    parser = argparse.ArgumentParser(description="Evaluate a trained XGBoost Ranking Model.")
    
    # Option 1: Specify all files manually
    parser.add_argument("--model-file", type=str, help="Path to the trained .joblib model pipeline file.")
    parser.add_argument("--eval-data-file", type=str, required=True, help="Path to the CSV data file for evaluation.")
    parser.add_argument("--config-file", type=str, help="Path to the preprocessing config.yaml file.")
    parser.add_argument("--sharpe-edges-file", type=str, help="Path to the .pkl file containing Sharpe quantile edges.")
    parser.add_argument("--feature-list-file", type=str, help="Path to the .pkl file containing the list of feature names.")
    
    # Option 2: Auto-discover files
    parser.add_argument("--model-dir", type=str, default="../model_output", 
                       help="Directory to auto-discover model files (default: ../model_output)")
    parser.add_argument("--auto-discover", action="store_true",
                       help="Automatically find latest model files in model-dir")

    args = parser.parse_args()
    
    # Auto-discovery logic
    if args.auto_discover or not all([args.model_file, args.config_file, args.sharpe_edges_file, args.feature_list_file]):
        logger.info("Auto-discovering model files...")
        model_file, feature_file, edges_file = find_latest_model_files(args.model_dir)
        
        if not all([model_file, feature_file, edges_file]):
            logger.error("Could not find all required model files. Please specify manually or check model directory.")
            sys.exit(1)
        
        args.model_file = model_file
        args.feature_list_file = feature_file
        args.sharpe_edges_file = edges_file
        
        # Use default config if not specified
        if not args.config_file:
            args.config_file = "config.yaml"
        
        logger.info(f"✅ Auto-discovered files:")
        logger.info(f"   Model: {os.path.basename(args.model_file)}")
        logger.info(f"   Features: {os.path.basename(args.feature_list_file)}")
        logger.info(f"   Edges: {os.path.basename(args.sharpe_edges_file)}")
        logger.info(f"   Config: {args.config_file}")
    
    # Validate all required files exist
    required_files = [args.model_file, args.eval_data_file, args.config_file, 
                     args.sharpe_edges_file, args.feature_list_file]
