...
class KellyConfig:
    ...
    # Position limits
    min_position_pct: float = 0.005  # 0.5% minimum (lowered)
    max_position_pct: float = 0.05   # 5% maximum per position (lowered for diversification)
    max_portfolio_risk: float = 0.50  # 50% max total risk (increased)

    # Risk of ruin constraints
    max_risk_of_ruin: float = 0.02   # 2% max probability of ruin (slightly relaxed)
    ruin_threshold: float = 0.50     # Define ruin as losing 50% of capital

    # Probability thresholds
    min_prob_to_trade: float = 0.45  # Don't trade if P(win) < this (lowered from 0.52)
    high_conviction_threshold: float = 0.55  # Increase size above this (lowered)
...
class KellySizer:
    ...
    def size_portfolio(
        ...
        - Diversification (via correlation penalty)
        ...
        Args:
            ...
            apply_correlation_penalty: Reduce sizes for correlated positions
            underlying_column: Column identifying underlying asset
        ...
        # Apply correlation penalty if requested
        if apply_correlation_penalty and underlying_column in df.columns:
            positions_with_size = df[df['position_size'] > 0]
            if len(positions_with_size) > 1:
                penalty = correlation_penalty(
                    positions_with_size,
                    underlying_column=underlying_column
                )
                if penalty < 1.0:
                    logger.info(f"Applying correlation penalty: {penalty:.2f}")
                    df.loc[df['position_size'] > 0, 'position_size'] *= penalty
