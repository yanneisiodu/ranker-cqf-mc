...
# For caching tsfresh features locally
...
def _engineer_features(df):
    ...
    if 'last' in df.columns and 'contractID' in df.columns:
        df['price_change_1d'] = df.groupby('contractID', group_keys=False)['last'].diff(1)
    else:
        df['price_change_1d'] = np.nan
        logger.warning("'last' or 'contractID' column missing, cannot calculate 'price_change_1d'.")

    # Calculate 1-day IV change
    if 'implied_volatility' in df.columns and 'contractID' in df.columns:
        df['iv_change_1d'] = df.groupby('contractID', group_keys=False)['implied_volatility'].diff(1)
    else:
        df['iv_change_1d'] = np.nan
        logger.warning("'implied_volatility' or 'contractID' column missing, cannot calculate 'iv_change_1d'.")
    ...
    # Zero-Day Premium Decay
    if 'theta' in df.columns and 'last' in df.columns and 'days_to_exp' in df.columns:
        denominator = df['last'] * np.sqrt(df['days_to_exp'] + 1e-8)
    ...
    # Mispricing Ratio
    if 'last' in df.columns and 'fair_value' in df.columns:
        df.loc[:, 'mispricing_ratio'] = np.where(df['fair_value'] == 0, 0,
                                                (df['last'] - df['fair_value']) / (df['fair_value'] + 1e-8))
    else:
...
def _add_tsfresh_features(df):
    """Helper to extract and merge tsfresh features without column conflicts."""
    logger.debug("Adding tsfresh features with conflict resolution...")
    logger.info("Extracting tsfresh features (caching disabled)")
    
    if {'contractID', 'date', 'last'}.issubset(df.columns):
        price_ts = df[['contractID', 'date', 'last']].copy()
        price_ts.rename(columns={'last': 'price'}, inplace=True)
        price_ts.dropna(subset=['price'], inplace=True)
        
        if not price_ts.empty:
            try:
                ts_features = extract_features(price_ts, column_id="contractID", column_sort="date",
                                               default_fc_parameters=MinimalFCParameters(), n_jobs=0,
                                               disable_progressbar=True)
                ts_features = ts_features.reset_index().rename(columns={'index': 'contractID'})
                logger.info("Extracted %d tsfresh features (caching disabled)", len(ts_features.columns))
                
                # CRITICAL FIX: Handle column conflicts before merging
                conflicting_columns = []
                for col in ts_features.columns:
                    if col in df.columns and col != 'contractID':
                        conflicting_columns.append(col)
                
                if conflicting_columns:
                    logger.info(f"Resolving {len(conflicting_columns)} column conflicts: {conflicting_columns}")
                    
                    # For conflicting columns, prefer tsfresh version for XGBoost-critical columns
                    for col in conflicting_columns:
                        if col in ['price__mean', 'price__standard_deviation']:
                            # These are critical for XGBoost - rename original and keep tsfresh
                            df = df.rename(columns={col: f'{col}_orig'})
                            logger.info(f"Renamed original '{col}' to '{col}_orig' to preserve tsfresh version")
                        else:
                            # For other conflicts, keep original and rename tsfresh
                            ts_features = ts_features.rename(columns={col: f'{col}_tsfresh'})
                            logger.info(f"Renamed tsfresh '{col}' to '{col}_tsfresh' to preserve original")
                
                # Now merge without conflicts
                df = df.merge(ts_features, on='contractID', how='left')
                logger.info("Merged tsfresh features into main dataframe without conflicts")
                
            except Exception as e:
                logger.error("tsfresh feature extraction failed: %s", e)
                ts_features = pd.DataFrame()
        else:
             ts_features = pd.DataFrame()
             logger.warning("No valid data for tsfresh feature extraction after cleaning.")
    else:
        ts_features = pd.DataFrame()
        logger.warning("Skipping tsfresh feature extraction: required columns missing ('contractID', 'date', 'last')")
...
def preprocess_data(df, config, scaler=None):
    ...
    # 3. Add tsfresh Features
    if config.get('use_tsfresh', True): # Optional tsfresh via config
        df = _add_tsfresh_features(df)
        if df.empty: logger.warning("DataFrame empty after tsfresh. Returning."); return df, scaler
    else:
        logger.info("Skipping tsfresh feature extraction based on config.")
