...
def calculate_target(df, lookahead_days=5):
    ...
    if 'contractID' not in df.columns or 'last' not in df.columns:
        logging.error("Cannot calculate target: 'contractID' or 'last' column missing after preprocessing.")
...
def train_ranking_model_with_optuna(data_files, config_file, optuna_trials:int, start_year:int, end_year:int, timestamp:str):
    ...
    if 'last' in df_processed.columns:
        last_shifted = df_processed.groupby('contractID')['last'].shift(1)
        df_processed['price_change_1d'] = np.where(last_shifted.fillna(0) != 0, (df_processed['last'] - last_shifted) / last_shifted, 0)
    else: df_processed['price_change_1d'] = 0
    if 'implied_volatility' in df_processed.columns:
        df_processed['iv_change_1d'] = df_processed.groupby('contractID')['implied_volatility'].diff(1)
    else: df_processed['iv_change_1d'] = 0
