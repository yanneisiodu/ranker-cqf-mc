...
def create_preprocessor(numerical_features, categorical_features, available_columns):
    logging.debug(f"Creating preprocessor with Imputer only (no scaling for tree models)...")
    transformers = []
    ...
    if num_features_exist:
        ...
        logging.debug(f"Preprocessor applying Imputer(mean) to {len(num_features_exist)} numerical features (no scaling for tree-based models).")
    else:
...
def train_ranking_model_with_optuna(data_files, config_file, optuna_trials:int, start_year:int, end_year:int, timestamp:str):
    ...
    try:
        ...
        # STEP 4 ACCEPTANCE GUARD: Ensure no globally-scaled columns leak into training
        scaled_cols = [c for c in X_final.columns if c.endswith('_scaled')]
        assert not scaled_cols, f"Found globally-scaled columns: {scaled_cols}. Remove dataset-wide scaling before training tree models."
        logging.info("✓ Acceptance guard passed: No globally-scaled columns detected")
