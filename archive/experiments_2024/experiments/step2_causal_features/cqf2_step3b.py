...
# Import Step 1: regime features, Step 2: time-decay weights, Step 3B: regime-conditional conformal
...
class OptimalCQF:
    ...
    def create_preprocessor(self, df: pd.DataFrame) -> Pipeline:
        ...
        cqf_features = [
            ...
            # Step 1: regime features + Step 2: time-decay weights + Step 3B: regime-conditional conformal
    ...
    def calculate_conformal_adjustments(self, alpha: float = 0.1) -> Dict[str, float]:
        """
        Step 3B: Regime-conditional adaptive conformal calibration.
        Uses light touch for stable periods, heavy protection for stress periods.
        """
        logger.info("Calculating regime-conditional conformal prediction adjustments")
    ...
    def save_model(self, output_path: str, metadata: Dict = None):
        ...
        # Save model artifacts (Step 3B: include regime-conditional conformal calibrator)
        model_artifact = {
            'models': self.models,
...
def load_and_prepare_data(data_file: str, config_file: str = "config.yaml") -> pd.DataFrame:
    ...
    logger.info("Added Step 1+2+3B enhancements: regime features + time-decay weights + regime-conditional conformal")
