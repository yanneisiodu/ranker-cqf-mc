...
class OptimalCQF:
    ...
    def create_preprocessor(self, df: pd.DataFrame) -> Pipeline:
        ...
        cqf_features = [
            ...
            # Step 1: regime features + Step 2: time-decay weights + Step 3B: regime-conditional conformal + Step 4: PageHinkley drift + Step 5: EVT tail protection + Step 6: VIX pipeline fix + enhanced regime detection
            'vix_regime', 'vol_cluster', 'stress_score',
            
            # Step 6: Enhanced regime detection features
            'realized_vol_20d', 'vol_of_vol_20d', 'vol_emergency', 'vol_acceleration'
        ]
    ...
    def calculate_conformal_adjustments(self, alpha: float = 0.1) -> Dict[str, float]:
        ...
        # Combined stress detection: trigger if ANY indicator shows stress
        is_stable_period = (vix_stress <= 1.0) and (not vol_emergency_active) and (vol_of_vol_stress <= 1.5)
        ...
        if 0.05 in predictions and 0.95 in predictions:
            if is_stable_period:
                ...
                logger.info(f"Stress period detected (VIX stress: {vix_stress:.3f}) - Using adaptive conformal + EVT")
                ...
                # Initialize and fit EVT adjuster with VIX-adaptive parameters
                self.evt_adjuster = EVTTailAdjuster(
                    tail_thresh=0.80,
                    base_alpha=0.005,      # Light tail mass for stable periods  
                    stress_alpha=0.02,     # Heavy tail mass for regime stress (VIX>25)
                    vix_threshold=25.0     # VIX level triggering stress mode
                )
    ...
    def predict_quantiles(self, df: pd.DataFrame, apply_conformal: bool = True) -> pd.DataFrame:
        ...
        if apply_conformal:
            ...
            if 'vol_of_vol_20d' in df.columns and len(df) > 0:
                ...
                vol_of_vol_threshold = df['vol_of_vol_20d'].quantile(0.90) if len(df) > 50 else 0
                vol_of_vol_high = vol_of_vol_current > vol_of_vol_threshold
            
            # Enhanced regime switching: use adaptive if ANY stress indicator active
            use_adaptive = (self.conformal_calibrator is not None) and (vol_emergency_now or vol_of_vol_high)
            
            if use_adaptive:
                # Stress detected: use adaptive conformal calibrator + EVT
                logger.info(f"STRESS DETECTED - Vol Emergency: {vol_emergency_now}, Vol-of-Vol High: {vol_of_vol_high} - Using adaptive conformal + EVT")
...
def main():
    ...
    try:
        ...
        if quality_passed:
            ...
            # Step 6 FIX: Add regime features to output for analysis
