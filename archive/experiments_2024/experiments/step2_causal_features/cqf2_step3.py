...
# Import Step 1: regime features, Step 2: time-decay weights, Step 3: adaptive conformal
from regime_tools import add_regime_features, calculate_time_decay_weights, AdaptiveConformalCalibrator
...
class OptimalCQF:
    ...
    def __init__(self, 
        ...
        self.conformal_calibrator = None  # Step 3: Adaptive conformal calibrator
    ...
    def create_preprocessor(self, df: pd.DataFrame) -> Pipeline:
        ...
        cqf_features = [
            ...
            # Step 1: regime features + Step 2: time-decay weights + Step 3: adaptive conformal
    ...
    def calculate_conformal_adjustments(self, alpha: float = 0.1) -> Dict[str, float]:
        """
        Step 3: Calculate adaptive conformal prediction adjustments with VIX×DTE grouping.
        ...
        # Extract regime features for grouping
        ...
        if 0.05 in predictions and 0.95 in predictions:
            # Step 3: Initialize and fit adaptive conformal calibrator
            self.conformal_calibrator = AdaptiveConformalCalibrator(
                alpha=alpha,
                use_groups=True,           # VIX×DTE grouping
                min_group_n=300,          # Minimum samples per group
                recency_lambda=0.99,      # Time decay
                median_debias=True        # Group-wise median bias correction
            )
        ...
        # Keep backward compatibility with simple adjustments dict
        self.conformal_adjustments = {'adaptive_conformal': True}
        return self.conformal_adjustments
    ...
    def predict_quantiles(self, df: pd.DataFrame, apply_conformal: bool = True) -> pd.DataFrame:
        """
        Step 3: Predict quantiles with adaptive conformal calibration.
        """
        ...
        # Apply Step 3: adaptive conformal adjustments
        if apply_conformal and self.conformal_calibrator is not None:
            # Extract regime features for adaptive adjustments
            ...
            # Apply adaptive conformal adjustments
            ...
            # Fallback to raw predictions (backward compatibility)
    ...
    def save_model(self, output_path: str, metadata: Dict = None):
        ...
        # Save model artifacts (Step 3: include adaptive conformal calibrator)
        model_artifact = {
            'models': self.models,
            'preprocessor': self.preprocessor,
            'feature_names': self.feature_names,
            'conformal_adjustments': self.conformal_adjustments,  # Backward compatibility
            'conformal_calibrator': self.conformal_calibrator,     # Step 3: Adaptive conformal
...
def load_and_prepare_data(data_file: str, config_file: str = "config.yaml") -> pd.DataFrame:
    ...
    # Step 1: Add regime features + Step 2: time-decay weights in training
    df_processed = add_regime_features(df_processed)
    logger.info("Added Step 1+2+3 enhancements: regime features + time-decay weights + adaptive conformal")
