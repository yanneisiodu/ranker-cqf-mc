...
def project_features_consistently(df, selected_features):
    """Project df to selected_features, adding missing cols with zeros, dropping extras."""
    missing_features = [f for f in selected_features if f not in df.columns]
    extra_features = [f for f in df.columns if f not in selected_features]
    
    if missing_features:
        logger.info(f"Adding {len(missing_features)} missing features with zeros: {missing_features[:5]}")
        for feat in missing_features:
            df[feat] = 0.0
    
    if extra_features:
        logger.info(f"Dropping {len(extra_features)} extra features: {extra_features[:5]}")
    
    return df[selected_features]

def defensive_ndcg_at_k(y_true_group, y_pred_scores, k, use_exp_gains=True):
    """Defensive NDCG computation with fallbacks."""
    try:
        if len(y_true_group) == 0 or k <= 0:
            return 0.0
        
        actual_k = min(k, len(y_true_group))
        
        if use_exp_gains:
            gains = (2 ** np.asarray(y_true_group)) - 1
        else:
            gains = np.asarray(y_true_group)
        ...
        # DCG calculation
        dcg = sum(g / np.log2(i + 2) for i, g in enumerate(gains_pred[:actual_k]))
        
        # IDCG calculation  
        ideal_gains = np.sort(gains)[::-1]
        idcg = sum(g / np.log2(i + 2) for i, g in enumerate(ideal_gains[:actual_k]))
        
        return dcg / max(idcg, 1e-12)
        
    except Exception as e:
        logger.warning(f"Defensive NDCG@{k} computation failed: {e}")
        return 0.0

def defensive_precision_at_k(y_true_group, y_pred_scores, k, relevance_threshold=3):
    """Defensive Precision@K computation with fallbacks."""
    try:
        if len(y_true_group) == 0 or k <= 0:
            return 0.0
        ...
        # Sort by predictions (descending)
        top_k_indices = np.argsort(y_pred_scores)[::-1][:actual_k]
        top_k_relevance = y_true_arr[top_k_indices]
        
        # Count relevant items (label >= threshold)
        num_relevant = np.sum(top_k_relevance >= relevance_threshold)
        return num_relevant / actual_k
        
    except Exception as e:
        logger.warning(f"Defensive Precision@{k} computation failed: {e}")
        return 0.0
...
def evaluate_model(args):
    ...
    X_eval_data = project_features_consistently(df_eval_with_target, final_features_to_use)
    ...
    ndcg_defensive_scores_by_k = {k: [] for k in K_VALUES}  # defensive NDCG with fallbacks
    precision_scores_by_k = {k: [] for k in K_VALUES}
    precision_defensive_scores_by_k = {k: [] for k in K_VALUES}  # defensive Precision with fallbacks
    ap_scores_by_k = {k: [] for k in K_VALUES}
    rank_ic_scores = []  # Spearman correlations per group
    ...
    for group_size in eval_group_info:
        ...
        for k_val in K_VALUES:
            ...
            # Defensive metrics with fallbacks
            ndcg_def_sc = defensive_ndcg_at_k(y_true_group, y_pred_group_scores, actual_k, use_exp_gains=True)
            ndcg_defensive_scores_by_k[k_val].append(ndcg_def_sc)
            
            prec_def_sc = defensive_precision_at_k(y_true_group, y_pred_group_scores, actual_k, relevance_threshold=3)
            precision_defensive_scores_by_k[k_val].append(prec_def_sc)

            ap_sc = calculate_average_precision_at_k(y_true_group, y_pred_group_scores, actual_k)
            ap_scores_by_k[k_val].append(ap_sc)
        
        # Rank IC (Spearman) per group
        try:
            if len(y_true_group) > 2:  # Need at least 3 points for meaningful correlation
                spearman_corr_group, _ = spearmanr(y_true_group, y_pred_group_scores)
                if not np.isnan(spearman_corr_group):
                    rank_ic_scores.append(spearman_corr_group)
        except Exception:
            pass  # Skip this group if correlation fails
    ...
    # Report Rank IC (Spearman per day)
    if rank_ic_scores:
        mean_rank_ic = np.mean(rank_ic_scores)
        median_rank_ic = np.median(rank_ic_scores)
        logger.info(f"Rank IC (Spearman per day) - mean: {mean_rank_ic:.4f}, median: {median_rank_ic:.4f} (from {len(rank_ic_scores)} days)")
    else:
        logger.warning("No valid Rank IC scores computed")
    ...
    for k_val in K_VALUES:
        ...
        # Defensive metrics
        mean_ndcg_defensive = np.nanmean(ndcg_defensive_scores_by_k[k_val]) if ndcg_defensive_scores_by_k[k_val] else np.nan
        logger.info(f"Mean NDCG(defensive exp gains)@{k_val}: {mean_ndcg_defensive:.4f}")
        ...
        mean_precision_defensive = np.nanmean(precision_defensive_scores_by_k[k_val]) if precision_defensive_scores_by_k[k_val] else np.nan
        logger.info(f"Mean Precision(defensive)@{k_val}: {mean_precision_defensive:.4f}")
