...
def _scale_data(df, scaler=None, config=None):
    ...
    if config is None:
        ...
        if not numerical_cols_from_config:
            logger.info("'numerical_cols_to_scale' not found in config or is empty - skipping dataset-wide scaling.")
    ...
    if not numerical_cols:
         logger.info("No numerical columns identified for scaling. Skipping scaling (tree models don't need scaling).")
         return df, scaler
