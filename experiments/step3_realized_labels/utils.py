...
def make_daily_rank_labels(df, h=3, price_col='last_raw', config=None):
    """
    Create per-day rank labels based on realized forward returns.
    
    Args:
        df (pd.DataFrame): Input DataFrame with contractID, date, and price columns
        h (int): Forward horizon in days (default: 3)
        price_col (str): Price column to use for returns calculation (default: 'last_raw')
        config (dict): Configuration with rank_label_bins settings
    
    Returns:
        pd.DataFrame: DataFrame with rank_label column added, rows with invalid targets removed,
                     and single-row days filtered out
    """
    logger.info(f"Computing per-day rank labels with h={h} days forward horizon using {price_col}")
    
    # Ensure data is sorted properly for shift operations
    df = df.sort_values(['contractID', 'date']).reset_index(drop=True)
    
    # Prefer midprice when available for more accurate pricing
    if 'bid' in df.columns and 'ask' in df.columns:
        # Check for valid bid/ask data
        valid_spread = (df['bid'] > 0) & (df['ask'] > 0) & (df['bid'] <= df['ask'])
        if valid_spread.sum() > len(df) * 0.5:  # If >50% have valid bid/ask
            df['midprice_raw'] = np.where(valid_spread, (df['bid'] + df['ask']) / 2, np.nan)
            # Fill NaN midprices with last_raw or last as fallback
            if 'last_raw' in df.columns:
                df['midprice_raw'] = df['midprice_raw'].fillna(df['last_raw'])
            elif 'last' in df.columns:
                df['midprice_raw'] = df['midprice_raw'].fillna(df['last'])
            
            price_col = 'midprice_raw'
            logger.info(f"Using midprice from bid/ask for {valid_spread.sum()} rows ({valid_spread.sum()/len(df)*100:.1f}%)")
    
    # Check if price column exists, with fallbacks
    if price_col not in df.columns:
        if 'last_raw' in df.columns:
            price_col = 'last_raw'
            logger.warning(f"Column 'midprice_raw' not found, falling back to 'last_raw'")
        elif 'last' in df.columns:
            price_col = 'last'
            logger.warning(f"Column 'last_raw' not found, falling back to 'last'")
        else:
            logger.error(f"No suitable price column found for target calculation")
            return df.assign(rank_label=np.nan)
    
    logger.info(f"Using price column: {price_col}")
    
    # Compute forward prices via groupby shift
    df['fwd_price'] = df.groupby('contractID')[price_col].shift(-h)
    
    # Compute target forward return with guards for NaN/zero
    df['target_fwd_return_{}d'.format(h)] = np.where(
        (df[price_col] > 0) & (~df[price_col].isna()) & (~df['fwd_price'].isna()),
        (df['fwd_price'] - df[price_col]) / df[price_col],
        np.nan
    )
    
    # Drop rows with invalid forward returns
    initial_count = len(df)
    df = df.dropna(subset=[f'target_fwd_return_{h}d'])
    valid_target_count = len(df)
    dropped_target = initial_count - valid_target_count
    logger.info(f"Dropped {dropped_target} rows with invalid forward returns ({dropped_target/initial_count*100:.1f}%)")
    
    if df.empty:
        logger.warning("No valid forward returns found after filtering")
        return df.assign(rank_label=np.nan)
    
    # Drop days with fewer than 2 rows (can't rank single-item days)
    daily_counts = df.groupby('date').size()
    valid_days = daily_counts[daily_counts >= 2].index
    
    before_day_filter = len(df)
    df = df[df['date'].isin(valid_days)]
    after_day_filter = len(df)
    dropped_single_days = before_day_filter - after_day_filter
    logger.info(f"Dropped {dropped_single_days} rows from single-row days ({len(daily_counts) - len(valid_days)} days removed)")
    
    if df.empty:
        logger.warning("No data remaining after filtering single-row days")
        return df.assign(rank_label=np.nan)
    
    # Get configurable bins from config
    if config is not None:
        bins_key = config.get('rank_label_bins_key', 'bins_a')
        bins = config.get(bins_key, [0.0, 0.50, 0.75, 0.90, 0.97, 1.0])
        logger.info(f"Using {bins_key} bins: {bins}")
    else:
        bins = [0.0, 0.50, 0.75, 0.90, 0.97, 1.0]  # Default 5-bin approach
        logger.info(f"Using default bins: {bins}")
    
    n_labels = len(bins) - 1
    
    # First pass: compute percentiles for weighting
    df['percentile'] = df.groupby('date')[f'target_fwd_return_{h}d'].transform(
        lambda x: x.rank(pct=True, method='first') if len(x) >= 2 else np.nan
    )
    
    def rank_within_day(group):
        """Rank contracts within a single day based on forward returns"""
        if len(group) < 2:
            return pd.Series(index=group.index, data=np.nan, dtype=float)
        
        # Compute percentiles for this day  
        returns = group[f'target_fwd_return_{h}d']
        percentiles = returns.rank(pct=True, method='first')
        
        # Bin into ordinal labels [0..n_labels-1] - HIGHER = BETTER convention
        labels = pd.cut(percentiles, bins=bins, labels=list(range(n_labels)), include_lowest=True)
        return labels.astype(float)
    
    # Apply ranking to get labels
    df['rank_label'] = df.groupby('date').apply(rank_within_day).droplevel(0)
    
    # Final cleanup: remove any remaining NaN labels
    final_before = len(df)
    df = df.dropna(subset=['rank_label'])
    final_after = len(df)
    df['rank_label'] = df['rank_label'].astype(int)
    
    logger.info(f"Final cleanup: dropped {final_before - final_after} rows with NaN rank labels")
    
    # Log coverage and distribution statistics
    total_coverage = final_after / initial_count if initial_count > 0 else 0
    unique_days = df['date'].nunique()
    logger.info(f"Overall coverage: {final_after}/{initial_count} ({total_coverage:.1%}) rows retained")
    logger.info(f"Final dataset: {final_after} rows across {unique_days} days")
    
    # Log rank label distribution
    rank_distribution = df['rank_label'].value_counts().sort_index()
    logger.info(f"Rank label distribution:\n{rank_distribution.to_string()}")
    
    # Sanity check: higher label should mean higher mean return (HIGHER = BETTER)
    sanity_check = df.groupby('rank_label')[f'target_fwd_return_{h}d'].mean().sort_index()
    logger.info(f"Sanity check - Mean return by rank_label (should be increasing):\n{sanity_check.to_string()}")
    
    # Add instance weights for top-tail focus
    if config is not None and config.get('use_instance_weighting', False):
        weight_scheme = config.get('weight_scheme', 'stepwise')
        
        if weight_scheme == 'stepwise':
            weights_config = config.get('stepwise_weights', {})
            p985_weight = weights_config.get('p985', 6.0)
            p95_weight = weights_config.get('p95', 3.0) 
            p85_weight = weights_config.get('p85', 2.0)
            base_weight = weights_config.get('base', 1.0)
            
            df['sample_weight'] = np.where(df['percentile'] >= 0.985, p985_weight,
                                  np.where(df['percentile'] >= 0.95, p95_weight,
                                  np.where(df['percentile'] >= 0.85, p85_weight, base_weight))).astype('float32')
            
            logger.info(f"Applied stepwise instance weighting: p>=98.5%: {p985_weight}x, p>=95%: {p95_weight}x, p>=85%: {p85_weight}x, base: {base_weight}x")
            
        elif weight_scheme == 'smooth':
            weights_config = config.get('smooth_weights', {})
            alpha = weights_config.get('alpha', 4.0)
            beta = weights_config.get('beta', 3.0)
            
            df['sample_weight'] = (1.0 + alpha * (df['percentile'] ** beta)).astype('float32')
            logger.info(f"Applied smooth instance weighting: w = 1 + {alpha} * (percentile^{beta})")
        
        # Log weight distribution
        weight_stats = df['sample_weight'].describe()
        logger.info(f"Sample weight statistics:\n{weight_stats.to_string()}")
    else:
        df['sample_weight'] = 1.0
        logger.info("No instance weighting applied")
    
    # Clean up temporary columns
    df = df.drop(columns=['fwd_price', 'midprice_raw', 'percentile'], errors='ignore')
