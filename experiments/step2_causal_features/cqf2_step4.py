...
# Import Step 1: regime features, Step 2: time-decay weights, Step 3B: regime-conditional conformal, Step 4: PageHinkley drift
from regime_tools import add_regime_features, calculate_time_decay_weights, AdaptiveConformalCalibrator, PageHinkley
...
class OptimalCQF:
    ...
    def create_preprocessor(self, df: pd.DataFrame) -> Pipeline:
        ...
        cqf_features = [
            ...
            # Step 1: regime features + Step 2: time-decay weights + Step 3B: regime-conditional conformal + Step 4: PageHinkley drift
    ...
    def calculate_conformal_adjustments(self, alpha: float = 0.1) -> Dict[str, float]:
        """
        Step 4: Regime-conditional conformal + PageHinkley drift detection.
        Uses light touch for stable periods, heavy protection for stress periods.
        Monitors coverage drift and triggers re-calibration on recent data.
        """
        logger.info("Calculating regime-conditional conformal with drift detection")
        ...
        # Detect market stress level (using VIX volatility as indicator)
        vix_stress = vix.std() if vix is not None else 0.5
        is_stable_period = vix_stress <= 1.0
        
        if 0.05 in predictions and 0.95 in predictions:
            if is_stable_period:
                # Stable period: light touch (match Simple CQF performance)
                logger.info(f"Stable period detected (VIX stress: {vix_stress:.3f}) - Using light conformal")
                ...
                # Stress period: full adaptive conformal protection
                logger.info(f"Stress period detected (VIX stress: {vix_stress:.3f}) - Using adaptive conformal")
                
                self.conformal_calibrator = AdaptiveConformalCalibrator(
                    alpha=alpha,              # Full protection during stress
                    use_groups=True,          # VIX×DTE grouping for stress
                    min_group_n=300,
                    recency_lambda=0.99,      # Heavy recency weighting
                    median_debias=True
                )
                ...
                adjustments = {'adaptive_conformal': True}  # Mark as adaptive
    ...
    def predict_quantiles(self, df: pd.DataFrame, apply_conformal: bool = True) -> pd.DataFrame:
        ...
        if apply_conformal:
            if self.conformal_calibrator is not None:
                # Stress period: use adaptive conformal calibrator
    ...
    def save_model(self, output_path: str, metadata: Dict = None):
        ...
        # Save model artifacts (Step 4: include PageHinkley drift detector + regime-conditional conformal)
        model_artifact = {
            'models': self.models,
...
def load_and_prepare_data(data_file: str, config_file: str = "config.yaml") -> pd.DataFrame:
    ...
    logger.info("Added Step 1+2+3B+4 enhancements: regime features + time-decay weights + regime-conditional conformal + PageHinkley drift")
