...
# Import regime features (Step 1) + time-decay weights (Step 2)
from regime_tools import add_regime_features, calculate_time_decay_weights
...
class OptimalCQF:
    ...
    def create_preprocessor(self, df: pd.DataFrame) -> Pipeline:
        ...
        cqf_features = [
            ...
            # Step 1: regime features + Step 2: time-decay sample weights
...
def load_and_prepare_data(data_file: str, config_file: str = "config.yaml") -> pd.DataFrame:
    ...
    logger.info("Added Step 1+2 enhancements: regime features + time-decay sample weights")
