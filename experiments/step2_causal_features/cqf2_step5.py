...
from regime_tools import add_regime_features, calculate_time_decay_weights, AdaptiveConformalCalibrator, PageHinkley, EVTTailAdjuster, _scores_one_sided
...
class OptimalCQF:
    ...
    def create_preprocessor(self, df: pd.DataFrame) -> Pipeline:
        ...
        cqf_features = [
            ...
            # Step 1: regime features + Step 2: time-decay weights + Step 3B: regime-conditional conformal + Step 4: PageHinkley drift + Step 5: EVT tail protection
...
def load_and_prepare_data(data_file: str, config_file: str = "config.yaml") -> pd.DataFrame:
    ...
    logger.info("Added Step 1+2+3B+4+5 enhancements: regime features + time-decay weights + regime-conditional conformal + PageHinkley drift + EVT tail protection")
